...
static const char *TAG = "DoubleNetworkBoard";

DoubleNetworkBoard::DoubleNetworkBoard(gpio_num_t ml307_tx_pin, gpio_num_t ml307_rx_pin, size_t ml307_rx_buffer_size) 
    : Board(), 
...
NetworkType DoubleNetworkBoard::LoadNetworkTypeFromSettings() {
...
void DoubleNetworkBoard::SaveNetworkTypeToSettings(NetworkType type) {
    Settings settings("network", true);
...
void DoubleNetworkBoard::InitializeCurrentBoard() {
...
void DoubleNetworkBoard::SwitchNetType() {
...
std::string DoubleNetworkBoard::GetBoardType() {
    return current_board_->GetBoardType();
}

void DoubleNetworkBoard::StartNetwork() {
...
Http* DoubleNetworkBoard::CreateHttp() {
    return current_board_->CreateHttp();
}

WebSocket* DoubleNetworkBoard::CreateWebSocket() {
    return current_board_->CreateWebSocket();
}

Mqtt* DoubleNetworkBoard::CreateMqtt() {
    return current_board_->CreateMqtt();
}

Udp* DoubleNetworkBoard::CreateUdp() {
    return current_board_->CreateUdp();
}

const char* DoubleNetworkBoard::GetNetworkStateIcon() {
    return current_board_->GetNetworkStateIcon();
}

void DoubleNetworkBoard::SetPowerSaveMode(bool enabled) {
    current_board_->SetPowerSaveMode(enabled);
}

std::string DoubleNetworkBoard::GetBoardJson() {   
