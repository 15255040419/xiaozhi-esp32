...
// 创建欢迎界面容器及其组件
void LcdDisplay::SetupWelcomeScreen(lv_obj_t* screen) {
    // 创建欢迎容器 - 直接放在屏幕上
    ...
    lv_label_set_text(date_label_, "1 周一");
    lv_obj_align(date_label_, LV_ALIGN_TOP_MID, 0, 100);
    
    // 计算时间标签的宽度
    lv_coord_t hour_width = lv_obj_get_width(hour_label_);
    lv_coord_t colon_width = lv_obj_get_width(colon_label_);
    lv_coord_t minute_width = lv_obj_get_width(minute_label_);
    lv_coord_t spacing = 5; // 标签之间的间距
    ...
    // 创建欢迎界面上的电池图标
    welcome_battery_label_ = lv_label_create(welcome_container_);
    lv_obj_set_style_text_font(welcome_battery_label_, fonts_.icon_font, 0);
    lv_obj_set_style_text_color(welcome_battery_label_, lv_color_white(), 0);
    lv_label_set_text(welcome_battery_label_, "");
    lv_obj_align(welcome_battery_label_, LV_ALIGN_TOP_RIGHT, -10, 5);
    
    // 创建欢迎界面上的WiFi图标
    welcome_network_label_ = lv_label_create(welcome_container_);
    lv_obj_set_style_text_font(welcome_network_label_, fonts_.icon_font, 0);
    lv_obj_set_style_text_color(welcome_network_label_, lv_color_white(), 0);
    lv_label_set_text(welcome_network_label_, "");
    lv_obj_align(welcome_network_label_, LV_ALIGN_TOP_RIGHT, -40, 5);
    
    // 创建欢迎界面上的静音图标
    welcome_mute_label_ = lv_label_create(welcome_container_);
    lv_obj_set_style_text_font(welcome_mute_label_, fonts_.icon_font, 0);
    lv_obj_set_style_text_color(welcome_mute_label_, lv_color_white(), 0);
    lv_label_set_text(welcome_mute_label_, "");
    ...
    // 强制立即更新一次时间显示，确保初始显示正确
    UpdateWelcomeTimeAndDate();
}

// 更新欢迎界面上的时间和日期
void LcdDisplay::UpdateWelcomeTimeAndDate() {
    DisplayLockGuard lock(this);
    ...
    if (date_label_ != nullptr && (day != last_day || timeinfo->tm_wday != last_wday || last_day == -1)) {
    ...
    // 更新欢迎界面上的状态图标
    UpdateWelcomeStatusIcons();
}

// 更新欢迎界面上的状态图标
void LcdDisplay::UpdateWelcomeStatusIcons() {
    // 同步欢迎界面上的电池和网络图标
